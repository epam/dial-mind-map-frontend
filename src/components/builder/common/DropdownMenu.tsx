...
import { hasParentWithAttribute } from '@/utils/app/modals';
...
export const MenuComponent = forwardRef<HTMLDivElement, MenuProps & HTMLProps<HTMLButtonElement>>(
  function MenuComponent(
    ...
    const { floatingStyles, refs, context } = useFloating<HTMLButtonElement>({
      ...
      onOpenChange: isOpened => {
        if (
          hasParentWithAttribute(context.dataRef.current.openEvent?.target as Element | null, 'data-no-context-menu')
        ) {
          return;
        }
    ...
    return (
      <FloatingNode id={nodeId}>
        ...
        >
          {trigger}
          {!trigger && label && <span className="inline-block truncate">{label}</span>}
        </div>
...
export const MenuItem = forwardRef<HTMLButtonElement, MenuItemProps & ButtonHTMLAttributes<HTMLButtonElement>>(
  function MenuItem({ className, label, item: ItemComponent, disabled, ...props }, forwardedRef) {
    ...
    return (
      <div>
        ...
        >
          {ItemComponent}
          {!ItemComponent && label && <span className="inline-block truncate">{label}</span>}
        </button>
      </div>
    );
  },
...
export const Menu = forwardRef<HTMLDivElement, MenuProps & HTMLProps<HTMLButtonElement>>(function Menu(props, ref) {
  const parentId = useFloatingParentNodeId();
  if (parentId === null) {
    return (
      <FloatingTree>
