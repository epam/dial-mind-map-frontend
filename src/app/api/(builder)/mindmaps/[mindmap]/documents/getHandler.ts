...
export const getDocumentsHandler = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn({ response: errRespText }, `Error happened during fetching documents for mindmap ${mindmapId}`);
    ...
    logger.error({ error: error }, `Error happened during fetching documents for mindmap ${mindmapId}`);
