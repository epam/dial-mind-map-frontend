  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn(
        ...
        `Error happened during fetching generation params for mindmap ${mindmapId}`,
      );
    ...
    logger.error({ error: error }, `Error happened during fetching generation params for mindmap ${mindmapId}`);
